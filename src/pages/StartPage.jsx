...
const StartPage = () => {
  ...
  useEffect(() => {
    const createParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const posY =
          type === "smoke" ? Math.random() * 30 + 70 : Math.random() * 100;
        ...
        const animation =
          type === "smoke"
            ? `smokeRise ${duration}s ease-out ${delay}s infinite`
            : `floatAnimation ${duration}s ease-in-out ${delay}s infinite`;

        particle.style.animation = animation;
        particle.style.filter = `blur(${
          type === "smoke" ? Math.random() * 2 + 1 : 0.5
        }px)`;
  ...
  useEffect(() => {
    const positionMagnifyingEffect = () => {
      ...
      const magnifyingEffect = document.querySelector(
        ".magnifying-effect-start"
      );
  ...
  const startGame = () => {
    ...
    document
      .querySelectorAll(
        ".black-card-start, .banana-character-start, .title-text-start, .banana-text-start, .mansion-text-start, .play-button-start"
      )
      .forEach((el) => {
  ...
  return (
    <div className="game-container-start">
      ...
      <div className="loading-text-start">Loading...</div>
    </div>
  );
};
